...
fun TipCalculator() {
    ...
    Column(modifier = Modifier.fillMaxWidth()) {
        TotalHeader(amount = formatTwoDecimalPoints(getTotalHeaderAmount(
            amount.value,
...
fun TotalHeader(amount:String) {
    Surface(modifier = Modifier
        .fillMaxWidth()
        .padding(12.dp), color = colorResource(id = R.color.aquamarine), shape = RoundedCornerShape(8.dp)
    ) {
        Column(
            modifier = Modifier
                .fillMaxWidth()
...
fun UserInputArea(
    ...
    tipPercentage:Float,
    tipPercentageChanege:(Float) -> Unit
) {
        ...
        Column(
            ...
            if(amount.isNotEmpty()){
                ...
                Row(modifier = Modifier
                    ...
                    Text(text = "₹ ${formatTwoDecimalPoints(getTipAmount(amount,tipPercentage))}", style = MaterialTheme.typography.titleMedium)
                }

                Spacer(modifier = Modifier.height(8.dp))

                Text(text = "${formatTwoDecimalPoints(tipPercentage.toString())} %", style = MaterialTheme.typography.titleMedium)

                Spacer(modifier = Modifier.height(8.dp))

                Slider(value = tipPercentage , onValueChange = {
                    tipPercentageChanege.invoke(it)
                }, valueRange = 0f..100f, steps = 49, modifier = Modifier
                    .padding(horizontal = 12.dp)
                    .fillMaxWidth())
